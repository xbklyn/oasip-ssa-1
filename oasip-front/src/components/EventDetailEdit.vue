<template>
  <div>
    ...
    <div class="l-w-824 h-36 mx-auto mt-24 grid justify-center">
      <div class="l-w-612">
        <h2 class="text-2xl mb-6">
          <span class="font-bold"> Step 1 </span>- Choose clinic
        </h2>
        <div class="grid grid-cols-2 place-items-center">
          <div class="justify-self-start">
            ...
            />
            <label
              :for="data.eventCategoryName"
              class="ml-2 text-sm font-light"
            >
              {{ data.eventCategoryName }}
            </label>
          </div>
          <p class="justify-self-start l-text-xxs font-semibold ml-3">
            {{ data.eventCategoryDuration }} Mins
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="l-w-824 h-70 mx-auto grid justify-center m-12">
      <div class="">
        <h2 class="text-2xl mb-6">
          <span class="font-bold"> Step 2 </span>- Fill infomation
        </h2>
      </div>
      <div class="space-y-6">
        <!-- Input - Email -->
    ...
    <div class="l-w-824 h-full mx-auto grid justify-center mt-12">
      <div class="">
        <h2 class="text-2xl mb-6">
          <span class="font-bold"> Step 3 </span>- Select time period
        </h2>
      </div>
      ...
      <div class="flex justify-self-end mt-12">
        <button
          @click="reset"
          class="text-blue-500 font-medium flex w-18 h-8 justify-self-end duration-150"
        >
          <svg
            ...
            aria-hidden="true"
            role="img"
            class="reset iconify iconify--system-uicons mr-1 stroke-1"
            width="24"
            height="24"
      ...
      <div
        class="mb-6 h-48 l-w-612 grid grid-cols-5 gap-6 mt-6 overflow-y-auto"
      >
        <button
          v-for="(time, index) in computeTimePeriod"
          :key="index"
    ...
    <div class="l-w-612 h-12 mx-auto my-12">
      <button
        @click="$emit('edit', currentData)"
        class="w-full h-full text-white duration-150 bg-emerald-400 hover:bg-emerald-600"
      >
        Update
      </button>
    </div>
          ...
          <div class="grid place-items-center gap-6">
            ...
            <div class="text-center">
              ...
              <p class="text-md l-color-gray-300">
                your information is already updated.
              </p>
            </div>
          </div>
        </div>
...
const getTime = computed(async () => {
  const temp = await getEventByCatAndDate(
    prop.data.categoryId,
    selectDate.value,
  );
  return temp;
});
...
const continueDate = computed(() => {
  ...
  const month = ref(
    (new Date(prop.data.eventStartTime).getMonth() + 1).toString(),
  );
...
const allEventStartTime = computed(() => {
  ...
  for (let i = 0; i < TimeBooked.value.length; i++) {
    bookedStartTime.value.push(
      new Date(TimeBooked.value[i].eventStartTime).toLocaleTimeString('th-TH'),
    );
  }
...
const isOverlap = (index) => {
  let start = new Date(
    new Date(currentDate.value).getFullYear(),
    new Date(currentDate.value).getMonth(),
    new Date(currentDate.value).getDate(),
    computeTimePeriod.value[index].startTime.split(':')[0],
    computeTimePeriod.value[index].startTime.split(':')[1],
  ).getTime();
  let cur = new Date().getTime();

  let START_TIME = new Date(
    new Date(selectDate.value).getFullYear(),
    new Date(selectDate.value).getMonth(),
    new Date(selectDate.value).getDate(),
    computeTimePeriod.value[index].startTime.split(':')[0],
    computeTimePeriod.value[index].startTime.split(':')[1],
  );
  let END_TIME = new Date(
    ...
    new Date(selectDate.value).getMonth(),
    new Date(selectDate.value).getDate(),
    computeTimePeriod.value[index].endTime.split(':')[0],
    computeTimePeriod.value[index].endTime.split(':')[1],
  );
...
const computeTimePeriod = computed(() => {
  ...
  while (i + CATE_DURATION.value < MAX) {
    ...
    TimePeriod.value.push({
      startTime: start.toLocaleTimeString('th-TH'),
      endTime: end.toLocaleTimeString('th-TH'),
    });
...
const indexOfTime = computed(() => {
  for (let i = 0; i < computeTimePeriod.value.length; i++) {
    if (
      computeTimePeriod.value[i].startTime >
      new Date(prop.data.eventStartTime).toLocaleTimeString('th-TH')
    ) {
      return i;
    }
  }
...
const timeContain = computed(() => {
  return allStartTime.value.value.indexOf(
    new Date(prop.data.eventStartTime).toLocaleTimeString('th-TH'),
  );
});
const startTime = ref(
  timeContain.value == -1 ? indexOfTime.value - 1 : timeContain.value,
);
